...
function Proizvod({ product, onAdd, onRemove, inCart }) {
  ...
  useEffect(() => {
    axios({
      method: "GET",
      url: "https://free.currconv.com/api/v7/convert?q=RSD_EUR&compact=ultra&apiKey=ccf4b20e3c691e9010c9",
    })
      .then((response) => {
  ...
  return (
    <div className={inCart === 1 ? "card" : "card-cart"} style={stil}>
      ...
      <div className="card-body">
        ...
        <p className="cenaRegularna">
          {" "}
          {product.price} RSD = {product.price * RSDEUR} EUR{" "}
        </p>
        <p className="cenaPopust">
          {" "}
          <strong> Cena: </strong> {product.price * 0.9} RSD ={" "}
          {product.price * RSDEUR * 0.9} EUR{" "}
        </p>
      </div>
      {inCart === 1 ? (
        <>
